...
namespace LazyStackAuthTests
{
    ...
    public class AuthTests
    {
        ...
        public AuthTests()
        {
            ...
            var awsSettings = AwsConfig.GetAwsSettings("default", "LazyStackAuthTests").GetAwaiter().GetResult();
            var awsSettingsStr = awsSettings.BuildJsonWrapped();
            var awsAmplifyConfig = new AwsAmplifyConfig(awsSettings);
            var awsAmplfiyConfigBodyStr = awsAmplifyConfig.BuildJson();
            var awsAmplifyClonfigStr = awsAmplifyConfig.BuildAwsExportsJs(awsAmplfiyConfigBodyStr);
            File.WriteAllText("aws-exports.js", awsAmplifyClonfigStr);
           
            //var awsSettingsStr = AwsConfig.GenerateSettingsJsonAsync("default", "LazyStackAuthTests").GetAwaiter().GetResult();

            // MethodMap.json file - this is in the same form generated by LazyStack MDD
            using var methodMapStream = Assembly.GetAssembly(typeof(AuthTests)).GetManifestResourceStream("LazyStackAuthTests.MethodMap.json");
            // UI Messages from the LazyStackAuth project
            ...
            IConfiguration appConfig = new ConfigurationBuilder()
                ...
                .AddJsonStream(new MemoryStream(Encoding.ASCII.GetBytes(awsSettingsStr)))
            ...
            services.AddSingleton<IAuthProcess, AuthProcess>(); 

            // LzHttpClient(IConfiguration, IAuthProvider)
            services.AddSingleton<ILzHttpClient, LzHttpClient>();
        ...
        [TestMethod]
        public async Task TestApiGateways()
        {
            ...
            var lzHttpClient = serviceProvider.GetService<ILzHttpClient>();
            var testClientSDK = new TestClientSDK(lzHttpClient); // NSwag is not generating interfaces so we have to instantiate
            ...
            authProcess.Login = login;
            Assert.IsTrue(await authProcess.VerifyLoginAsync() == AuthEventEnum.AuthChallenge);
            // VerifyPasswordAsync - this completes the Sign In process
            authProcess.Password = password;
            Assert.IsTrue(await authProcess.VerifyPasswordAsync() == AuthEventEnum.SignedIn);

            // Make Call to HttpApiSecure (uses JWT Authorization header)
            Assert.IsTrue((await testClientSDK.GetUserHttpApiAsync(System.Threading.CancellationToken.None)).Length == 36);

            // Make Call to ApiSecure (Uses signed header)
            Assert.IsTrue((await testClientSDK.GetUserApiAsync(System.Threading.CancellationToken.None)).Length == 36);

            // Make Call to ApiSecure to test JSON return type (Uses signed header)
            Assert.IsTrue((await testClientSDK.GetTestJsonApiAsync(System.Threading.CancellationToken.None)).Field1 == "Field1 is yada");
        ...
        { 
            ...
            // Test Reset Password - Start the Rest Password Process
            ...
            Assert.IsTrue(authProcess.IsNewEmailVerified == false, "IsNewEmailVerified==false");
            Assert.IsTrue(authProcess.NewEmailNotVerified == true, "NewEmailNotVerified==true");
            Assert.IsTrue(authProcess.IsPasswordFormatOk == false, "IsPasswordFormatOk==true");
            Assert.IsTrue(authProcess.IsPasswordVerified == false, "IsPasswordVerified==false");
            Assert.IsTrue(authProcess.PasswordNotVerified == true, "PasswordNotVerified==true");
